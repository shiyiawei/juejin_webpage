<template>
  <nav class="nav-view">
    <div class="nav-mian shadow" :class="[{ 'nav-main--sticky': !isTopbarBlock }, isWhite ? 'topBarDay' : 'night']">
      <ul class="nav-list">
        <li class="nav-item" :class="{ 'nav-item--active': item.category_url === paramsTitle }" v-for="item in channels"
          :key="item.category_id" @click="navItemClick(item)" ref="navItem">
      ...
      <div class="show" ref="show">
        <el-dropdown @command="handleCommand">
          ...
          <el-dropdown-menu slot="dropdown">
            <el-dropdown-item :command="item" ref="navItem" v-for="item in arr" :key="item.category_id">{{
              item.category_name }}</el-dropdown-item>
          </el-dropdown-menu>
        </el-dropdown>
      </div>
    </div>
