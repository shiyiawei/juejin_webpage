    ...
    <div>
        <div class="index-container">
                ...
                <div class="list__header">
                    <ul class="list__nav">
                        <li class="list__nav-item" :class="{ 'list__nav-item--active': item.id == navId }"
                            v-for="item in navs" :key="item.title" @click="changeNavType(item)">
                            {{ item.title }}
                        </li>
                        <el-select v-if="navTypes.length" size="mini" style="width:100px" v-model="sort_type"
                            placeholder="请选择" @change="changeNavType">
                            <el-option v-for="item in navTypes" :key="item.title" :label="item.title"
            ...
            <div class="index-side">
                <ad v-for="item in bannerList" :key="item.id" :banner="item"></ad>
                <download></download>
                <author :list="authorList"></author>
                <info></info>
            </div>
        </div>
    </div>
...
import ad from '~/components/layout/mainright/ad'
import author from '~/components/layout/mainright/author'
import download from '~/components/layout/mainright/download'
import info from '~/components/layout/mainright/info'
...
export default {
    ...
    created() {
        console.log(this.isWhite, 'iswhite');
    },
    computed: {
